...
function JGraph(pointTypeX, pointTypeY) {
	var checkType = function(type) {
		if (type === undefined) {
			return JGraph.PointType.NoError;
		} else if (typeof type === 'number') {
			if (type < JGraph.PointType.NoError || type > JGraph.PointType.AsymmetricError) {
	...
	const pointDataLength = function(tx, ty) {
		var f = function(type) {
			if (type == JGraph.PointType.NoError) {
				return 1;
			} else if (type == JGraph.PointType.SymmetricError) {
				return 2;
			} else if (type == JGraph.PointType.AsymmetricError) {
				return 3;
			}
		};
		return f(tx) + f(ty);
	...
	var pointFormat = {
		...
		xerr : {
			show : true,
			asymmetric : (typeX == JGraph.PointType.AsymmetricError),
			upperCap : "-",
			lowerCap : "-"
		},
		yerr : {

 ...
 * 
 * Authors: Soki Sakurai Date: 2013-**-**
 * 
 ******************************************************************************/
...
(function() {
    ...
    HSQuickLook.JGraph.JGraph = function(pointTypeX, pointTypeY) {
        ...
        var checkType = function(type) {
            if (type === undefined) {
                ...
                if (type < PointType.NoError
                        || type > PointType.AsymmetricError) {
        ...
        const
        typeX = checkType(pointTypeX);
        const
        typeY = checkType(pointTypeY);

        const
        pointDataLength = function(tx, ty) {
            var f = function(type) {
                if (type == PointType.NoError) {
                    return 1;
                ...
                $(queryToPlaceholder)
                        .bind(
                                ...
                                function(event, ranges) {
                                    ...
                                    range_locked = true;
                                    $.plot(queryToPlaceholder, [ plotData ], $
                                            .extend(true, {}, options, rxy));
                                });
                var selection_str = "";
                if (enadisX) {
                    selection_str = "x";
        ...
        this.setPointFormat = function(show, radius, color) {
            ...
            plotData.points = pointFormat;
        };

        this.setPointColor = function(color) {
            pointFormat.color = color;
            plotData.points = pointFormat;            
        };

        this.setPointSize = function(size) {
            pointFormat.radius = size;
            plotData.points = pointFormat;            
        };
        ...
        this.setLogScaleX = function(enadis) {
            ...
            if (logScaleX) {
                options.xaxis.ticks = [ 0.00001, 0.0001, 0.001, 0.01, 0.1, 1,
        ...
        this.setData = function(d) {
            if (d instanceof Array) {
                if (d[0].length == pointDataLength) {
                    ...
                    throw "Data array with invalid length: " + d
                            + " is pushed into JGraph.";
                }
            } else {

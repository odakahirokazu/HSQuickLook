 ...
 * HS Quick Look
 * 
 * Authors: Hirokazu Odaka, Soki Sakurai Date: 2012-10-14 (alpha) Date:
 * 2014-04-07 (v0.5.1)
 * 
 ******************************************************************************/
...
(function() {
    ...
    $(document).ready(
            function() {
                ...
                bindEventForms();
                $.getJSON("user_data/user_configuration.json").done(initialize)
                        .fail(function() {
    ...
    function loadDataSheetList() {
        ...
        var title = $("<option />").html("").attr("value", "").attr("label",
                "Select data sheet");
        target.append(title);
        for (key in schema) {
    ...
    function updateTable(tableInfo, data, ti) {
        ...
        for ( var key in contents) {
            ...
            if (info.type == "trend-graph") {
                ...
                for ( var i = 0; i < info.group.length; i++) {
                    ...
                    var convertedValue = convertValue(plotInfo, values[source]);
                    ...
                    // graphs[fullKey].enableRescale(true, false);
                    graphs[sourceID].pushData([ time, Number(convertedValue) ]);
                    graphs[sourceID].enableRescale(true, false);
                    var lastpoint = graphs[sourceID].getLastPoint();
                    var yvalue = 0;
                    if (lastpoint != void 0) {
                        yvalue = lastpoint[1];
                    }
                    if (!graphs[sourceID].isRangeLocked()) {
                        while (yMaxs[sourceID] <= yvalue + 0.1) {
                            yMaxs[sourceID] = 1.2 * yMaxs[sourceID] + 1;
                        }
                        var g = keyToGraphs[sourceID][1];
                        var gKey = keyToGraphs[sourceID][0];
                        if (graphsToDraw[gKey] == void 0) {
                            graphsToDraw[gKey] = [ g,
                                    [ time - (xWidths[fullKey]+0.5), time + 0.5 ],
                                    [ -0.1, yMaxs[sourceID] ] ];
                        } else {
                            if (graphsToDraw[gKey][2][1] < yMaxs[sourceID]) {
                                graphsToDraw[gKey][2][1] = yMaxs[sourceID];
                            }
                        }
                    }
                }
        ...
        for ( var key in graphsToDraw) {
            graphsToDraw[key][0].enableRescale(true, false);
            graphsToDraw[key][0].plot(graphsToDraw[key][1],
                    graphsToDraw[key][2]);
        }
        return true;
    }
     ...
     * Data element
     */
    function setIfGiven(value, func) {
        if (value != void 0) {
            func(value);
        }
    }
    function makePair(key, rawValue, info, tableID) {
        ...
        if (type == "trend-graph") {
            ...
            if (multiGraphs[elemID] == void 0) {
                ...
                multiGraphs[elemID] = new HSQuickLook.JMultiGraph.JMultiGraph();
                multiGraphs[elemID].enableRescale(true, false);
                multiGraphs[elemID].setQueryToPlaceholder("#" + elemID
                        + "_placeholder");
                elemValue.attr("id", elemID);
                elemValue.append(cont);
                // multiGraphs[groupName].addGraph(g);
                // keyToGraphs[elemID] = [ groupName, multiGraphs[groupName] ];
            ...
            // var container = createPlotContainer(elemID);
            ...
            for ( var i = 0; i < info.group.length; i++) {
                ...
                if ('options' in plot) {
                    ...
                    setIfGiven(opt.color, g.setPointColor);
                    setIfGiven(opt.color, g.setLineColor);
                    setIfGiven(opt.pointSize, g.setPointSize);
                }
                graphs[sourceID] = g;
                yMaxs[sourceID] = 0.0;
                // tOrigins[sourceID] = -1.0;
                multiGraphs[elemID].addGraph(g);
    ...
    // function createJGraph(key, capacity) {
    function createJGraph(capacity) {
        var g = new HSQuickLook.JGraph.JGraph(
                HSQuickLook.JGraph.PointType.NoError,
                HSQuickLook.JGraph.PointType.NoError);
        // g.setDataLabel(key);
    ...
    function formatValue(info, value) {
        ...
        var valueFormated = (format === void 0) ? value
    ...
    function displayImages() {
        $(".display_phase0").removeClass("display_phase0").addClass(
                "display_phase1");
    }
    ...
    function createPlotContainer(plotid) {
        var dc = $("<div />").attr("id", plotid + "_graph").addClass(
                "demo-container");
        var ph = $("<div />").attr("id", plotid + "_placeholder").addClass(
                "demo-placeholder");
        dc.append(ph);

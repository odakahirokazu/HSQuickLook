 ...
 * Date: 2025-07-14 | Hirokazu Odaka | review, clearData()
 *
 ******************************************************************************/
...
(function () {
  ...
  let TrendCurve = HSQuickLook.graph.TrendCurve;
  ...
  TrendCurve.prototype.clearData = function () {
    let data = this.data;
    data.x.length = 0;
    data.y.length = 0;
  };
  ...
  HSQuickLook.graph.MultiTrendCurves = function () {
        ...
        },
        {
          ...
          click: function (gd) {
            if (gd.layout.yaxis.type === "linear") {
              ...
              let r1 = CheckLogCondition(gd.layout.yaxis.range);
              gd.layout.yaxis.range[0] = Math.log10(r1[0]);
              gd.layout.yaxis.range[1] = Math.log10(r1[1]);

              let r2 = CheckLogCondition([gd.layout.yMin, gd.layout.yMax]);
              gd.layout.yMin = Math.log10(r2[0]);
              gd.layout.yMax = Math.log10(r2[1]);
    ...
    this.clearData = function () {
      for (let curve of Object.values(this.trendCurves)) {
        curve.clearData();
      }
    };
    ...
    this.plot = function () {
      ...
      if (counter == this.refreshPhase) {
        let range = [this.layout.yMin, this.layout.yMax];
        if (this.layout.yaxis.type === "log") {
          range = [10 ** range[0], 10 ** range[1]];
        }
        for (let curve in this.trendCurves) {
          let value = this.trendCurves[curve].getLastYValue();
  ...
  let MultiTrendCurves = HSQuickLook.graph.MultiTrendCurves;
  ...
  MultiTrendCurves.prototype.setRangeY = function (range) {
    if (this.layout.yaxis.type === "log") {
      let ret = CheckLogCondition(range);
  ...
  MultiTrendCurves.prototype.adjustRangeX = function (x) {
    if (this.layout.autoMove === false) {
      return;
    }
    if (this.layout.xaxis.type === "log") {
      let ret = CheckLogCondition([x - this.xWidth + 0.5, x + 0.5]);
  ...
  MultiTrendCurves.prototype.adjustRangeY = function (y) {
    let range = GetAppropriateRangeY(this.layout.yaxis.range, y);
    if (range === null) {
      return;
    }
    this.setRangeY(range);
...
CheckLogCondition = function (range) {
  let yMax_negative = 100;
  let yMin_negative = 0.1;
  let min_value = range[0];
